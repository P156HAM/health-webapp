...
import { getMockHealthReports } from '../mocks/services'
...
const useFetchLongTermReports = (userUid: string | undefined, trigger: number) => {
  ...
  async function fetchHealthReportsForDoctor(uid: string | undefined): Promise<HealthReports[]> {
    const q = query(
    ...
    try {
      ...
      for (const doc of querySnapshot.docs) {
        ...
        const reportCreatedAt = data.created_at.toDate().toLocaleDateString('sv-SE')
        const dateOfBirth = data.date_of_birth.toDate().toLocaleDateString('sv-SE')
        const reportSharedAt = data.shared_at.toDate().toLocaleDateString('sv-SE')
  ...
  useEffect(() => {
    const fetchHealthReportsDataForDoctor = async () => {
      try {
        if (isMockMode) {
          const mockReports = await getMockHealthReports()
          setHealthReports(mockReports)
          setNoPatients(mockReports.length === 0)
          setIsLoading(false)
          return
        }
        if (!userUid) {
          setError('Failed to fetch health reports. Please check the provided id.')
          return
        }
